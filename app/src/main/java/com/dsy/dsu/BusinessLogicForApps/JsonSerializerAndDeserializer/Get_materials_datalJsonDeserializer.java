 ...
 return  РезультатОперацииBurkUPDATE.size();
        ...
        try{
            this.context=context;
                ...
                String  SQlOperInsert=  " REPLACE INTO "+имяТаблицаAsync+
                        " (  type_material, nomen_vesov,tracks,companys,count, date_update, uuid, user_update  , cfo, current_table  ,status_send ,ttn,datattn )  "
                        +" VALUES (?,?,?,?,?,?,?,?,?,?,"+
                        " ?,?,?) ;";
    ...
    private SQLiteStatement методGetSqliteStatementForInsert(@NonNull SQLiteDatabase Create_Database_СамаБАзаSQLite,
        ...
        try{
            ...
            sqLiteStatementInsert.bindLong(1, jsonNodeParentMAP.get("type_material").intValue());//"uuid"
            sqLiteStatementInsert.bindLong(2, jsonNodeParentMAP.get("nomen_vesov").intValue());//"uuid"


            // TODO: 25.09.2024 "tracks"
            if (  jsonNodeParentMAP.has("tracks") ) {
                if (!jsonNodeParentMAP.get("tracks").isNull()  ) {
                    sqLiteStatementInsert.bindLong(3, jsonNodeParentMAP.get("tracks").intValue());//"uuid"
                }else {
            ...
            sqLiteStatementInsert.bindLong(4, jsonNodeParentMAP.get("companys").intValue());//"uuid"
            sqLiteStatementInsert.bindLong(5, jsonNodeParentMAP.get("count").intValue());//"uuid"
            ...
            sqLiteStatementInsert.bindLong(11, jsonNodeParentMAP.get("status_send").intValue());//"uuid"


            // TODO: 25.09.2024 "ttn"
            if (  jsonNodeParentMAP.has("ttn") ) {
                if (!jsonNodeParentMAP.get("ttn").isNull()  ) {
            ...
            if (jsonNodeParentMAP.has("datattn")) {
                sqLiteStatementInsert.bindString(13, jsonNodeParentMAP.get("datattn").asText());//"date_update"
            }else {
                sqLiteStatementInsert.bindNull(13);
            }
