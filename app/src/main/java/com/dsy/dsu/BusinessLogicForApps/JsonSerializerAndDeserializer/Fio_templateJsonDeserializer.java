 ...
 return  РезультатОперацииBurkUPDATE.size();
        ...
        try{
            this.context=context;
                ...
                String  SQlOperInsert=  "REPLACE INTO "+имяТаблицаAsync
                        + " (  uuid,  fio_template,fio_uuid  , date_update,user_update ,current_table ,status_send)  "
                        +" VALUES(?,?,?, ?,?,? ,? );";
    ...
    private SQLiteStatement методGetSqliteStatementForInsert(@NonNull SQLiteDatabase Create_Database_СамаБАзаSQLite,
        ...
        try{
            ...
            sqLiteStatementInsert.bindLong(2, jsonNodeParentMAP.get("fio_template").longValue());//"uuid"
            sqLiteStatementInsert.bindLong(3, jsonNodeParentMAP.get("fio_uuid").longValue());//"uuid"
            ...
            // TODO: 25.09.2024 "status_send"
            if (  jsonNodeParentMAP.has("status_send") ) {
                if (!jsonNodeParentMAP.get("status_send").isNull()  ) {
                    sqLiteStatementInsert.bindLong(7, jsonNodeParentMAP.get("status_send").intValue());//"user_update"
                }else {
    ...
    private SQLiteStatement методGetSqliteStatementForUpdate(@NonNull SQLiteDatabase Create_Database_СамаБАзаSQLite,
        ...
        try{
            ...
            if (jsonNodeParentMAP.has("status_send")) {
                if (!jsonNodeParentMAP.get("status_send").isNull()) {
                    sqLiteStatementInsert.bindString(7, jsonNodeParentMAP.get("status_send").asText().trim());//"user_update"
                }else {
                    sqLiteStatementInsert.bindNull(7);
                }
            }else {
