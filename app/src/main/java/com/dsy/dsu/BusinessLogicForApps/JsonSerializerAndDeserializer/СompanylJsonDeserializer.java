 ...
 return  РезультатОперацииBurkUPDATE.size();
        ...
        try{
            this.context=context;
                ...
                String  SQlOperInsert=  " REPLACE INTO "+имяТаблицаAsync+"  " +
                        "  (     name ,fullname,inn, kpp  , date_update ,user_update,uuid,current_table   )  " +
                        " VALUES (?,?,?,?,?,?,?,? ) ;";
    ...
    private SQLiteStatement методGetSqliteStatementForInsert(@NonNull SQLiteDatabase Create_Database_СамаБАзаSQLite,
        ...
        try{
            ...
            // TODO: 25.09.2024 HAS fullname
            if (  jsonNodeParentMAP.has("fullname") ) {
                if (!jsonNodeParentMAP.get("fullname").isNull()) {
            ...
            // TODO: 25.09.2024 HAS inn
            if (  jsonNodeParentMAP.has("inn") ) {
                if (!jsonNodeParentMAP.get("inn").isNull()) {
                    sqLiteStatementInsert.bindString(3, jsonNodeParentMAP.get("inn").asText().trim());//"date_update"
                } else {
            ...
            // TODO: 25.09.2024 HAS inn
            if (  jsonNodeParentMAP.has("kpp") ) {
                if (!jsonNodeParentMAP.get("kpp").isNull()) {
                    sqLiteStatementInsert.bindString(4, jsonNodeParentMAP.get("kpp").asText().trim());//"date_update"
                } else {
