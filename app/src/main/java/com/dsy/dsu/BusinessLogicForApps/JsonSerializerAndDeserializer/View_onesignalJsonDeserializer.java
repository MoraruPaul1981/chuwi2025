        ...
        try{
            this.context=context;
                ...
                String  SQlOperInsert=  " REPLACE INTO "+имяТаблицаAsync+
                        " (  uuid,onesignal, current_table,user_update,date_update )  "
                        +" VALUES (?,?,?,?,? ) ;";
    ...
    private SQLiteStatement методGetSqliteStatementForInsert(@NonNull SQLiteDatabase Create_Database_СамаБАзаSQLite,
        ...
        try{
           ...
           // sqLiteStatementInsert.bindLong(1, jsonNodeParentMAP.get("id").intValue());//"id""
            ...
            sqLiteStatementInsert.bindString(2, jsonNodeParentMAP.get("onesignal").asText().trim());//"date_update"
            sqLiteStatementInsert.bindLong(3, jsonNodeParentMAP.get("current_table").longValue());//"current_table"
